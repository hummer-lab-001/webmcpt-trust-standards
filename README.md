...
| `catalog/language-style-guide-patterns.md` | Google's Python, C++, Java, Go, TypeScript, Shell, JavaScript, JSON, HTML/CSS, Markdown, and C# style guides read directly, with the guide's own stated rationale for each restriction — the "restrict the language for a stated scale/correctness reason" pattern, and how it echoes the project-governance module-boundary pattern above. Verbatim rules are separated from paraphrased ones, and each entry records what was *not* read. |
