...
| `catalog/webmcp-and-webmcpt.md` | Reads the W3C Community Group's **WebMCP** specification from raw source and answers one question: does it address who may be trusted? Partially — it names the trust gap in its own words ("Agents must assume good faith from site developers") and records that no verification mechanism exists, while its three mitigations all act on the payload rather than the counterparty. Includes the scope of what was and was not read. No official relationship; the one-letter name difference is a coincidence of subject matter. |
