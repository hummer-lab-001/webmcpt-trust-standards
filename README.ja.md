...
> 一節がニコラ・テスラの言葉として流通しています。**一次資料はありません。** 最も古い
> 記録でもO'Neillの1944年の伝記における**又聞きの回想**であり、テスラ自身の著作・講演・
> 同時代のインタビューには存在しません（3という数へのこだわり自体は別途、より確かな記録があります）。
> ここで触れるのは、これがこのカタログの主題そのものの標本だからです——**「誰が言ったとされるか」
> だけで大規模に反復され、誰も出典を確かめなかった主張**。「Googleがそうしているから」と同じ形です。
> 目指している違いはひとつ：このリポジトリの数字は、全部あなたが自分で数えられます。
